...
- Managed SSH key verification now matches the generated key's RouterOS row by
  comment/fingerprint and runs OpenSSH with `IdentitiesOnly=yes` plus `-F /dev/null`,
  preventing older user keys or agent/config identities from producing a false verified
  result.
