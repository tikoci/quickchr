...
- **A healthy large download was reported as a timeout** (#116). Both download paths
  ...
  then 101.5 s** on consecutive attempts over the same link — straddling the flat
  120 s, so an unchanged healthy download passed or failed on link jitter alone. A transfer
