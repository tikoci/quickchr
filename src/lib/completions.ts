...
_quickchr() {
    ...
    local commands="add start stop list ls status inspect env get set remove rm clean console exec qga license disk snapshot snap networks net cache doctor settings setup completions version help"
    ...
    local cmd="${D}{words[1]}"
    case "${D}cmd" in
        start|remove|rm|clean|console|exec|qga|license|disk|snapshot|snap|status|inspect|env|get|set)
            local machines
        ...
        completions)
            ...
            ;;
        cache)
            local opts="add key list prune clear"
            COMPREPLY=( $(compgen -W "${D}opts" -- "${D}cur") )
            ;;
        help)
            COMPREPLY=( $(compgen -W "${D}commands" -- "${D}cur") )
    ...
    case ${D}state in
        command)
            local commands=(
                ...
                'inspect:Print running machine descriptor'
                'env:Print subprocess environment'
                'get:Show live machine config (license, device-mode, admin)'
                'set:Change a booted machine (license, device-mode)'
                ...
                'cache:Prefetch and manage downloaded CHR images'
                'doctor:Check prerequisites'
                'settings:Manage global quickchr settings'
        ...
        args)
            ...
            case ${D}cmd in
                ...
                completions)
                    ...
                    ;;
                cache)
                    local subcommands=(add key list prune clear)
                    _describe 'cache subcommand' subcommands
                    ;;
                help)
                    local commands=(add start stop list status inspect env get set remove clean console exec qga license disk snapshot networks cache doctor settings setup completions version)
                    _describe 'command' commands
                    ;;
            esac
            ;;
...
set -l quickchr_commands add start stop list ls status inspect env get set remove rm clean console exec qga license disk snapshot snap networks net cache doctor settings setup completions version help
...
complete -c quickchr -n "not __fish_seen_subcommand_from $quickchr_commands" \\
    -a "inspect"     -d "Print running machine descriptor"
complete -c quickchr -n "not __fish_seen_subcommand_from $quickchr_commands" \\
    -a "env"         -d "Print subprocess environment"
complete -c quickchr -n "not __fish_seen_subcommand_from $quickchr_commands" \\
...
complete -c quickchr -n "not __fish_seen_subcommand_from $quickchr_commands" \\
    -a "get"         -d "Show live machine config (license, device-mode, admin)"
complete -c quickchr -n "not __fish_seen_subcommand_from $quickchr_commands" \
    -a "set"         -d "Change a booted machine (license, device-mode)"
complete -c quickchr -n "not __fish_seen_subcommand_from $quickchr_commands" \
...
complete -c quickchr -n "not __fish_seen_subcommand_from $quickchr_commands" \\
    -a "cache"       -d "Prefetch and manage downloaded CHR images"
complete -c quickchr -n "not __fish_seen_subcommand_from $quickchr_commands" \\
    -a "doctor"      -d "Check prerequisites"
complete -c quickchr -n "not __fish_seen_subcommand_from $quickchr_commands" \\
    -a "settings"    -d "Manage global quickchr settings"
complete -c quickchr -n "not __fish_seen_subcommand_from $quickchr_commands" \\
...
complete -c quickchr -n "__fish_seen_subcommand_from start remove rm clean console exec qga license disk snapshot snap status inspect env get set" \\
...
# cache subcommands
complete -c quickchr -n "__fish_seen_subcommand_from cache" -a "add key list prune clear" -d "cache subcommand"
`;
}
