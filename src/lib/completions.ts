...
_quickchr() {
    ...
    local commands="add start stop list ls status inspect env remove rm clean console exec qga license disk snapshot snap networks net cache doctor settings setup completions version help"
    ...
    local cmd="${D}{words[1]}"
    case "${D}cmd" in
        start|remove|rm|clean|console|exec|qga|license|disk|snapshot|snap|status|inspect|env)
            local machines
        ...
        args)
            ...
            case ${D}cmd in
                ...
                cache)
                    ...
                    ;;
                help)
                    local commands=(add start stop list status inspect env remove clean console exec qga license disk snapshot networks cache doctor settings setup completions version)
                    _describe 'command' commands
                    ;;
            esac
            ;;
...
set -l quickchr_commands add start stop list ls status inspect env remove rm clean console exec qga license disk snapshot snap networks net cache doctor settings setup completions version help
...
complete -c quickchr -n "__fish_seen_subcommand_from start remove rm clean console exec qga license disk snapshot snap status inspect env" \\
