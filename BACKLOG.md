...
- [ ] [P3] **Windows — triage the informational CI integration job** — an opt-in, non-gating `windows-integration` job now exists in Extended Verification (`continue-on-error: true`, TCG, `choco install qemu`; added in place of the original "local-first" plan since no Windows host is available to validate on). **Next:** run it (start `test-filter: start-stop.test.ts` to prove boot, then widen) and triage results — TCP channels under load, SLiRP networking, CHR boot timing under TCG, and the `+6..+8` IPC vs `extraPorts` collision. Known expected failures: `sshpass`/`socat` absent on Windows (scp + named-socket paths). Use the captured `integration-output.txt` artifact + step summary to decide real fixes vs documenting SLiRP-only mode.
