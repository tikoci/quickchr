...
- arm64 guests on macOS auto-select TCG (`-cpu cortex-a710`) — HVF cannot run the CHR image's 32-bit ARM userspace on Apple Silicon (DESIGN.md #10, quickchr#97). `--accel`/`QUICKCHR_ACCEL` overrides this for testing.
